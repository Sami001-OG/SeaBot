...
function InteractivePreview({ inline, className, children }: any) {
  ...
  const isReact = match && (language === 'tsx' || language === 'jsx' || language === 'react' || language === 'javascript' || language === 'typescript');
  const codeContent = String(children).replace(/\n$/, '');

  useEffect(() => {
    // If it's a standard code block, autonomously store it in the memory vault
    if (!inline && !isHtml && !isReact && !hasSaved && codeContent.trim().length > 0) {
  ...
  }, [inline, isHtml, isReact, hasSaved, codeContent, language]);

  if (!inline && (isHtml || isReact)) {
    // Inject Tailwind CDN seamlessly so the agent has styles out of the box
    // Also inject React + Babel if the block is React code
    const srcDoc = `
      <!DOCTYPE html>
      <html lang="en">
        <head>
        ...
        <body>
          ${isReact ? `
            ...
            <script type="text/babel" data-type="module">
              ...
              // Map lucide icons if the user attempts to import them
              window.lucide = window.lucide || {};
              
              // Handle relative imports commonly generated by LLMs
              const ReactModule = React;
              
              try {
                 // Try parsing standard export default
                 ${codeContent.replace(/import .* from .*;/g, '') // Strip imports
                 ...
                 // Fallback if App wasn't assigned via export default but a component exists
                 const ComponentToRender = window.App || (typeof App !== 'undefined' ? App : (typeof Main !== 'undefined' ? Main : null));
                 
                 if (ComponentToRender) {
                    const root = ReactDOM.createRoot(document.getElementById('root'));
                    root.render(React.createElement(ComponentToRender));
                    setTimeout(() => { if(window.lucide && window.lucide.createIcons) window.lucide.createIcons(); }, 100);
                 } else {
                    document.getElementById('root').innerHTML = '<div style="color:red; font-family:monospace; padding:20px;">Could not find a default export component to render.</div>';
                 }
              } catch(e) {
                 document.getElementById('root').innerHTML = '<div style="color:red; font-family:monospace; padding:20px;"><b>Render Error:</b> ' + e.message + '</div>';
              }
            </script>
          ` : codeContent}
        </body>
